#line 1 "Drivers\\STM32F4xx_HAL_Driver\\Src\\stm32f4xx_hal_timebase_rtc_alarm_template.c"
...
#line 66 "Drivers\\STM32F4xx_HAL_Driver\\Src\\stm32f4xx_hal_timebase_rtc_alarm_template.c"
...
#line 96 "Drivers\\STM32F4xx_HAL_Driver\\Src\\stm32f4xx_hal_timebase_rtc_alarm_template.c"
...
#line 133 "Drivers\\STM32F4xx_HAL_Driver\\Src\\stm32f4xx_hal_timebase_rtc_alarm_template.c"
   
  RCC_OscInitStruct.OscillatorType = ((uint32_t)0x00000001U);
  RCC_OscInitStruct.PLL.PLLState = ((uint8_t)0x00U);
  RCC_OscInitStruct.HSEState = ((uint8_t)0x01U);
   
  PeriphClkInitStruct.RTCClockSelection = ((uint32_t)((uint32_t)0x00000300U | (uint32_t)((((uint32_t)25000000U)/1000000U) << 16U)));
  ...
  {
    PeriphClkInitStruct.PeriphClockSelection = ((uint32_t)0x00000002U);
    if(HAL_RCCEx_PeriphCLKConfig(&PeriphClkInitStruct) == HAL_OK)
    {
      ...
      do{ (&hRTC_Handle)->Instance ->WPR = 0xCAU; (&hRTC_Handle)->Instance ->WPR = 0x53U; } while(0);

       
      ((&hRTC_Handle)->Instance ->CR &= ~(0x00000100U));

       
      ((&hRTC_Handle)->Instance ->ISR) = (~((((uint32_t)0x00000100U)) | 0x00000080U)|((&hRTC_Handle)->Instance ->ISR & 0x00000080U));

      counter = 0U;
       
      while((((((&hRTC_Handle)->Instance ->ISR) & (((uint32_t)0x00000001U))) != RESET)? SET : RESET) == RESET)
      {
      ...
      ((&hRTC_Handle)->Instance ->CR |= (((uint32_t)0x00001000U)));

       
      (((EXTI_TypeDef *) ((0x40000000U + 0x00010000U) + 0x3C00U))->IMR |= ((uint32_t)0x00020000U));
      (((EXTI_TypeDef *) ((0x40000000U + 0x00010000U) + 0x3C00U))->RTSR |= ((uint32_t)0x00020000U));

       
      if((hRTC_Handle.Instance->ISR & 0x00000040U) == (uint32_t)RESET)
      {
         
        hRTC_Handle.Instance->ISR = (uint32_t)((uint32_t)0xFFFFFFFFU);
        counter = 0U;
        while((hRTC_Handle.Instance->ISR & 0x00000040U) == (uint32_t)RESET)
        {
          if(counter++ == (SystemCoreClock /48U))  
          {
            return HAL_ERROR;
      ...
      hRTC_Handle.Instance->ISR &= (uint32_t)~0x00000080U;

       
      do{ (&hRTC_Handle)->Instance ->WPR = 0xFFU; } while(0);
...
void HAL_RTC_AlarmAEventCallback(RTC_HandleTypeDef *hrtc)
{
  ...
  do{ (hrtc)->Instance ->WPR = 0xCAU; (hrtc)->Instance ->WPR = 0x53U; } while(0);

   
  hrtc->Instance->ISR = (uint32_t)((uint32_t)0xFFFFFFFFU);

  while((hrtc->Instance->ISR & 0x00000040U) == (uint32_t)RESET)
  {
    if(counter++ == (SystemCoreClock /48U))  
    {
      break;
  ...
  hrtc->Instance->ISR &= (uint32_t)~0x00000080U;

   
  do{ (hrtc)->Instance ->WPR = 0xFFU; } while(0);
}
